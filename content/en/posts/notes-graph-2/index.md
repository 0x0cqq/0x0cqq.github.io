...
> 2. 另一个方向略显困难：如果原来的 $n-1$ 列线性无关，那么删掉一行之后这 $n-1$ 行仍然线性无关吗？是的。从线性代数视角来看，如果删掉某行后不是线性无关，那么存在列的线性组合使得到的列中该行元素不是 $0$，该行之外的元素全都是 $0$ 。那么关联矩阵的这 $n-1$ 列的所有元素和不为 $0$ ；这与关联矩阵中每一列所有元素和都是 $0$ 矛盾。
